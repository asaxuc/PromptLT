...
class ModifiedResidualAttentionBlock_MMA(nn.Module):
...
class Transformer_MMA(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None):
        ...
        self.resblocks = nn.Sequential(
            *[
                ModifiedResidualAttentionBlock_MMA(width, heads, index + 1, attn_mask)
...
class VisionTransformer_MMA(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int):
        ...
        self.transformer = Transformer_MMA(width, layers, heads)
...
class ResidualAttentionBlock_PLT(nn.Module):
    ...
    def attention(self, x: torch.Tensor):
        ...
        '''
        if self.attn_mask.shape[0] != x.shape[1]:
            ...
            return mask
            return self.attn(x, x, x, need_weights=False, attn_mask=torch.)[0]
        '''
        if len(x.shape) == 4:
            N,B,C,D = x.shape
            x = x.reshape(x.shape[0],-1,x.shape[-1])
            temp = self.attn(x, x, x, need_weights=False, attn_mask=self.attn_mask)[0]
            return temp.reshape(temp.shape[0], B, C, temp.shape[-1])
        else:
            return self.attn(x, x, x, need_weights=False, attn_mask=self.attn_mask)[0]
...
class VisionTransformer_PLT(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int,
                 design_details):
        super().__init__()
        self.input_resolution = input_resolution
...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int,
        ...
        self.output_dim = output_dim
        self.conv1 = nn.Conv2d(in_channels=3, out_channels=width, kernel_size=patch_size, stride=patch_size, bias=False)
        if design_details["trainer"] != "maple" or design_details["vision_depth"] == 0:
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            elif trainer == "MultiModalAdapter":
        ...
        if trainer == "MultiModalAdapter":
            self.transformer = Transformer_MMA(
